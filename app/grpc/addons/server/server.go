...
func (s *Server) Run() error {
	...
	go func() {
		zap.S().Infof("[GRPCSERVER] Starting gRPC Server at addr %s", grpcl.Addr().String())
		zap.S().Fatalw("[GRPCSERVER] gRPC Server failed:", s.NewGRPCServer(false).Serve(grpcl))
	}()
	go func() {
		gwMux := s.NewGatewayMux(httpl, false)
		zap.S().Infof("[PROXYSERVER] Sarting Proxy Server at port %s", httpl.Addr().String())
		srv := &http.Server{
		...
		zap.S().Fatalw("[PROXYSERVER] Proxy Server failed:", srv.Serve(httpl))
	}()
...
func (s *Server) ServeHTTPS() {
	...
	*/
	...
	zap.S().Infow("[HTTP2] Starting HTTP2 Server at port", l.Addr().String())
	zap.S().Fatalw("[HTTP2] HTTP2 Server failed:", srv.Serve(tls.NewListener(l, tlsConfig)))
}
